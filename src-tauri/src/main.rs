...
fn detect_mice(show_hidden: bool) -> Result<Vec<MouseDevice>, String> {
    #[cfg(target_os = "windows")]
    {
        ...
        Ok(mice
            .into_iter()
            .filter(|mouse| show_hidden || is_relevant_mouse(mouse))
            .collect())
    }
...
fn is_relevant_mouse(mouse: &MouseDevice) -> bool {
    ...
    if HIDDEN_DEVICE_TERMS.iter().any(|term| text.contains(term)) {
        return false;
    }

    if GAMING_BRANDS.iter().any(|brand| text.contains(brand)) {
        return true;
    }

    mouse.vid.is_some()
        && mouse.manufacturer.is_some()
...
mod windows_mouse_detection {
    ...
    pub fn detect() -> Result<Vec<MouseDevice>, String> {
        ...
        loop {
            ...
            let hardware_id = registry_property(device_info_set, &device_info, SPDRP_HARDWAREID)
            ...
            let manufacturer = registry_property(device_info_set, &device_info, SPDRP_MFG)
