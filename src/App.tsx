...
import { Gauge, Gamepad2, Keyboard, Mouse, Palette, Play, Plus, RefreshCw, Settings, SlidersHorizontal, Trash2, Upload, Zap } from "lucide-react";
...
type ButtonAction = "Default" | "Open File Explorer" | "Open Task Manager" | "Open Windows Settings" | "Back" | "Forward" | "DPI Up" | "DPI Down" | "Custom program" | "Disabled";
...
const actions: ButtonAction[] = ["Default", "Open File Explorer", "Open Task Manager", "Open Windows Settings", "Back", "Forward", "DPI Up", "DPI Down", "Custom program", "Disabled"];
...
function DpiInput({ value, onApply, compact = false }: { value: number; onApply: (value: number) => void; compact?: boolean }) {
  const [text, setText] = useState(String(value));
  useEffect(() => setText(String(value)), [value]);
  const commit = () => {
    const parsed = Number(text.replace(/[^0-9]/g, ""));
    const dpi = Number.isFinite(parsed) ? Math.min(40000, Math.max(50, Math.round(parsed / 50) * 50)) : value;
    setText(String(dpi));
  ...
  return <div className={`app-shell glass-${glassMode}`} style={glassStyle}>
    ...
    <div className="ui-scale-layer" style={{ "--ui-scale": uiScale / 100 } as React.CSSProperties}>
      ...
      <main className="content">
        ...
        {tab === "overview" && <><section className="device-hero panel glass-surface"><div className="device-hero-copy"><div className="status-pill glass-control"><span className={`device-dot ${connected ? "online" : ""}`}/>{connected ? "Connected" : "Not connected"}</div><h2>{mouse?.name || "No mouse detected"}</h2><p>{connected ? "Connected and ready" : "Connect a mouse to begin configuring it"}</p><div className="device-meta">{mouse?.connection && <span>{mouse.connection}</span>}{mouse?.manufacturer && <span>{mouse.manufacturer}</span>}{mouse?.vid && mouse?.pid && <span>VID {mouse.vid} · PID {mouse.pid}</span>}</div><button className="primary-button glass-control" onClick={() => void refresh()} disabled={loading} type="button"><RefreshCw size={16}/> {loading ? "Scanning…" : "Scan for devices"}</button></div><div className="mouse-illustration"><img src="/assets/x1/attack-shark-x1-top.png" alt="White Attack Shark X1, top view"/></div></section><div className="section-heading"><div><h2>Quick settings</h2><p>Common controls for your current profile.</p></div></div><section className="settings-grid"><div className="setting-card panel glass-surface"><div className="setting-icon"><Gauge size={19}/></div><div className="setting-copy"><span>DPI</span><DpiInput value={dpi} onApply={applyDpi} compact/><small>Hardware sensitivity</small></div><input type="range" min="50" max="40000" step="50" value={dpi} onChange={e => applyDpi(Number(e.target.value))}/><div className="dpi-preset-row">{dpiPresets.slice(0, 4).map(value => <button className={dpi === value ? "selected" : ""} key={value} onClick={() => applyDpi(value)} type="button">{value >= 1000 ? `${value / 1000}K` : value}</button>)}</div><small className="muted-warning">Hardware DPI control · 50–40,000 DPI.</small></div><div className="setting-card panel glass-surface"><div className="setting-icon"><Zap size={19}/></div><div className="setting-copy"><span>Polling rate</span><strong>{polling}</strong><small>USB report rate</small></div><div className="segmented">{["125 Hz","500 Hz","1000 Hz"].map(r => <button className={polling === r ? "selected" : ""} key={r} onClick={() => setPollingForProfile(r)} type="button">{r.replace(" Hz","")}</button>)}</div></div><div className="setting-card panel glass-surface"><div className="setting-icon"><Keyboard size={19}/></div><div className="setting-copy"><span>Button layout</span><strong>5 buttons</strong><small>Ready to customise</small></div><button className="secondary-button glass-control" onClick={() => setTab("buttons")} type="button">Configure</button></div></section></>}
        {tab === "buttons" && <section className="panel page-panel glass-surface"><div className="section-heading compact"><div><h2>Button mapping</h2><p>Saved to {activeProfile?.name || "this profile"}. Test an action before you use it.</p></div><div className="page-status"><span className="device-dot online"/>Profile saved</div></div><div className="mapping-label-row"><span>Button</span><span>Control</span><span>Assigned action</span><span>Test</span></div><div className="button-map-grid">{buttonNames.map(button => { const binding = buttons[button] || { action: "Default" as ButtonAction }; return <div className="map-row glass-control" key={button}><span className="button-key">{button.replace("Button ","M")}</span><span>{button}</span><select value={binding.action} onChange={e => updateButton(button, { action: e.target.value as ButtonAction, target: e.target.value === "Custom program" ? binding.target || "" : undefined })}>{actions.map(action => <option key={action}>{action}</option>)}</select>{binding.action === "Custom program" && <input className="map-target" value={binding.target || ""} placeholder="Program path, e.g. notepad.exe" onChange={e => updateButton(button, { target: e.target.value })}/>}<button className="map-test" onClick={() => void testButtonAction(binding)} type="button" title="Test this action"><Play size={14}/></button></div>; })}</div><p className="mapping-note">Actions are saved per profile. Explorer, Task Manager, Settings, custom programs, and Disabled are applied to side buttons M4/M5 while Unnamed is running; use the test button to verify them first. Hardware-native remapping stays separate until the X1’s command format is captured.</p></section>}
        {tab === "performance" && <section className="panel page-panel glass-surface"><div className="section-heading compact"><div><h2>Performance</h2><p>Fine-tune sensitivity and responsiveness.</p></div></div><div className="performance-layout"><div className="performance-value"><span>Current DPI</span><DpiInput value={dpi} onApply={applyDpi}/><small>Hardware DPI control · 50–40,000 DPI.</small></div><div className="performance-slider"><input className="big-range" type="range" min="50" max="40000" step="50" value={dpi} onChange={e => applyDpi(Number(e.target.value))}/><div className="dpi-preset-row full">{dpiPresets.map(value => <button className={dpi === value ? "selected" : ""} key={value} onClick={() => applyDpi(value)} type="button">{value >= 1000 ? `${value / 1000}K` : value}</button>)}</div></div></div><div className="polling-options"><span>Polling rate</span>{["125 Hz","500 Hz","1000 Hz"].map(r => <button className={polling === r ? "selected" : ""} key={r} onClick={() => setPollingForProfile(r)} type="button">{r}</button>)}</div><div className="dpi-diagnostics"><button className="secondary-button glass-control" onClick={() => void inspectDpiHardware()} type="button">Collect DPI diagnostics</button><p>Read-only: this records the mouse’s HID interfaces and descriptor without changing its settings.</p>{dpiDiagnostics && <pre>{dpiDiagnostics}</pre>}</div></section>}
