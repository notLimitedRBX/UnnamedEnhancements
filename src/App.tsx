...
import { Gauge, Gamepad2, Keyboard, Mouse, Palette, Play, Plus, RefreshCw, Settings, SlidersHorizontal, Sparkles, Trash2, Upload, Zap } from "lucide-react";
...
type Tab = "overview" | "buttons" | "performance" | "profiles" | "assistant" | "settings";
...
const tabs: { id: Tab; label: string; icon: typeof Mouse }[] = [
  ...
  { id: "performance", label: "Performance", icon: Gauge }, { id: "profiles", label: "Profiles", icon: Gamepad2 }, { id: "assistant", label: "Assistant", icon: Sparkles }, { id: "settings", label: "Settings", icon: Settings },
];
...
export default function App() {
  ...
  const askAssistant = async () => {
    setError(null); setAssistantReply(""); setAssistantLoading(true);
    try {
      const deviceContext = mouse ? `${mouse.name} · ${mouse.connection} · profile ${activeProfile?.name || "Default"} · ${dpi} DPI` : "No device detected";
      setAssistantReply(await invoke<string>("ask_personal_assistant", { apiKey: assistantKey, message: assistantQuestion, deviceContext }));
  ...
  return <div className={`app-shell glass-${glassMode}`} style={glassStyle}>
    ...
    <div className="ui-scale-layer" style={{ "--ui-scale": uiScale / 100 } as React.CSSProperties}>
      ...
      <main className="content">
        ...
        {tab === "assistant" && <section className="panel page-panel glass-surface assistant-panel"><div className="section-heading compact"><div><h2>Personal Assistant</h2><p>Ask about Unnamed, your mouse, or everyday Windows help.</p></div><span className="assistant-badge"><Sparkles size={14}/> OpenAI</span></div><div className="assistant-card glass-control"><label>OpenAI API key<input className="assistant-key" type="password" value={assistantKey} placeholder="sk-…" autoComplete="off" onChange={event => setAssistantKey(event.target.value)}/></label><small>Your key stays in this session only and is sent directly to OpenAI for each answer.</small><label>What do you need help with?<textarea value={assistantQuestion} placeholder="For example: What DPI should I use for everyday work?" onChange={event => setAssistantQuestion(event.target.value)} onKeyDown={event => { if ((event.ctrlKey || event.metaKey) && event.key === "Enter") void askAssistant(); }}/></label><div className="assistant-actions"><button className="primary-button glass-control" disabled={assistantLoading || !assistantQuestion.trim() || !assistantKey.trim()} onClick={() => void askAssistant()} type="button"><Sparkles size={16}/>{assistantLoading ? "Thinking…" : "Ask assistant"}</button><span>Ctrl + Enter to send</span></div>{assistantReply && <article className="assistant-reply"><span>Assistant</span><p>{assistantReply}</p></article>}</div></section>}
