...
export default function App() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => changeTab("performance"), 450);
  ...
  return <div className={`app-shell glass-${glassMode}`} style={glassStyle}>
    ...
    <div className="ui-scale-layer" style={{ "--ui-scale": uiScale / 100 } as React.CSSProperties}>
      ...
      <main className="content">
        ...
        {tab === "trainer" && <section className="panel page-panel glass-surface trainer-panel"><div className="section-heading compact"><div><h2>Quick aim trial</h2><p>A standalone 30-second practice space for your current DPI.</p></div><span className="trainer-dpi">{dpi} DPI</span></div><div className="trainer-mode-row">{(["Flick","Tracking","Reaction"] as TrainerMode[]).map(mode => <button className={trainerMode === mode ? "selected" : ""} onClick={() => { setTrainerMode(mode); setTrainerActive(false); }} key={mode} type="button"><b>{mode}</b><span>{mode === "Flick" ? "Fast target clicks" : mode === "Tracking" ? "Moving target practice" : "Quick response"}</span></button>)}</div><div className="trainer-stage"><div className="trainer-score"><span>Hits <b>{trainerScore}</b></span><span>Time <b>{trainerSeconds}s</b></span></div>{trainerActive ? <button className="trainer-target" onClick={hitTrainerTarget} style={{ left: `${trainerTarget.x}%`, top: `${trainerTarget.y}%` }} type="button" aria-label="Aim target" /> : <div className="trainer-start"><strong>{trainerSeconds === 0 ? `${trainerScore} hits — nice run.` : "Ready when you are."}</strong><span>Click the targets as quickly as you can.</span><button className="primary-button" onClick={beginTrainer} type="button">Start 30-second trial</button></div>}</div><p className="trainer-note">Practice happens only inside Unnamed. It does not read, modify, or automate any game.</p></section>}
