...
export default function App() {
  ...
  const askLocalAssistant = async () => {
    setError(null); setLocalReply(""); setLocalLoading(true);
    try {
      const deviceContext = mouse ? `${mouse.name} · ${mouse.connection} · profile ${activeProfile?.name || "Default"} · ${dpi} DPI` : "No device detected";
      setLocalReply(await invoke<string>("ask_local_assistant", { message: localQuestion, deviceContext }));
  ...
  return <div className={`app-shell glass-${glassMode}`} style={glassStyle}>
    ...
    <div className="ui-scale-layer" style={{ "--ui-scale": uiScale / 100 } as React.CSSProperties}>
      ...
      <main className="content">
        ...
        {tab === "assistant" && <section className="panel page-panel glass-surface local-ai-panel"><div className="section-heading compact"><div><h2>Local AI</h2><p>A private, free assistant running through Ollama on this PC.</p></div><span className="local-ai-badge"><Bot size={14}/> Offline-ready</span></div><div className="local-ai-card glass-control"><div className="local-ai-setup"><strong>One-time setup</strong><p>Install Ollama, then open PowerShell and run:</p><code>ollama pull qwen2.5:3b-instruct</code><small>Once downloaded, this assistant runs locally — no API key or usage fees.</small></div><label>What do you need help with?<textarea value={localQuestion} placeholder="For example: How do I make a work profile with 800 DPI?" onChange={event => setLocalQuestion(event.target.value)} onKeyDown={event => { if ((event.ctrlKey || event.metaKey) && event.key === "Enter") void askLocalAssistant(); }}/></label><div className="local-ai-actions"><button className="primary-button glass-control" disabled={localLoading || !localQuestion.trim()} onClick={() => void askLocalAssistant()} type="button"><Bot size={16}/>{localLoading ? "Thinking…" : "Ask Local AI"}</button><span>Ctrl + Enter to send</span></div>{localReply && <article className="local-ai-reply"><span>Local AI</span><p>{localReply}</p></article>}</div></section>}
