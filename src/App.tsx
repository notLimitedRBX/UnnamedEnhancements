...
type Tab = "overview" | "dashboard" | "buttons" | "performance" | "profiles" | "tools" | "tester" | "dpiConverter" | "sensitivity" | "diagnostics" | "surface" | "backup" | "autoSwitch" | "help" | "support" | "assistant" | "settings";
...
const tabs: { id: Tab; label: string; icon: typeof Mouse }[] = [
  ...
  { id: "tester", label: "Mouse tester", icon: Mouse }, { id: "dpiConverter", label: "DPI converter", icon: Gauge }, { id: "sensitivity", label: "Sensitivity converter", icon: SlidersHorizontal }, { id: "diagnostics", label: "Device diagnostics", icon: RefreshCw }, { id: "surface", label: "Surface check", icon: Zap }, { id: "backup", label: "Profile backup", icon: BookOpen }, { id: "autoSwitch", label: "Auto-switching", icon: Play },
...
const sidebarGroups: { label: string; ids: Tab[] }[] = [
  ...
  { label: "Tools", ids: ["tester", "dpiConverter", "sensitivity", "diagnostics", "surface", "backup", "autoSwitch"] },
  ...
  return <div className={`app-shell glass-${glassMode}`} style={glassStyle}>
    ...
    <div className="ui-scale-layer" style={{ "--ui-scale": uiScale / 100 } as React.CSSProperties}>
      <aside className="sidebar glass-surface">
        ...
        <div className="sidebar-spacer"/><button className={`nav-item settings-nav ${tab === "settings" ? "active" : ""}`} onClick={() => changeTab("settings")} type="button"><Settings size={18}/><span>Settings</span></button><div className="sidebar-device-card glass-surface"><span className={`device-dot ${connected ? "online" : ""}`}/><div className="device-card-copy"><strong>{mouse?.name || "No mouse detected"}</strong><span>{mouse?.connection || "Waiting for device"}</span></div><button className="icon-button" onClick={() => void refresh()} type="button"><RefreshCw size={15} className={loading ? "spin" : ""}/></button></div>
      ...
      <main className="content">
        ...
        {tab === "dpiConverter" && <section className="panel page-panel glass-surface"><div className="section-heading compact"><div><h2>DPI converter</h2><p>Keep the same effective sensitivity while moving to a new DPI value.</p></div></div><article className="tool-card glass-control"><div className="converter-grid"><label>Current DPI<input type="number" min="50" value={converterDpi} onChange={e => setConverterDpi(Math.max(50, Number(e.target.value) || 50))}/></label><label>Current sensitivity<input type="number" min="0.001" step="0.01" value={converterSensitivity} onChange={e => setConverterSensitivity(Math.max(.001, Number(e.target.value) || .001))}/></label><label>New DPI<input type="number" min="50" value={targetDpi} onChange={e => setTargetDpi(Math.max(50, Number(e.target.value) || 50))}/></label></div><div className="converter-result"><span>New sensitivity</span><strong>{(converterDpi * converterSensitivity / targetDpi).toFixed(3)}</strong></div></article></section>}
        {tab === "sensitivity" && <section className="panel page-panel glass-surface"><div className="section-heading compact"><div><h2>Sensitivity converter</h2><p>Calculate a matching in-game sensitivity for your preferred DPI.</p></div></div><article className="tool-card glass-control"><div className="converter-grid"><label>Old DPI<input type="number" min="50" value={converterDpi} onChange={e => setConverterDpi(Math.max(50, Number(e.target.value) || 50))}/></label><label>Old sensitivity<input type="number" min="0.001" step="0.01" value={converterSensitivity} onChange={e => setConverterSensitivity(Math.max(.001, Number(e.target.value) || .001))}/></label><label>Target DPI<input type="number" min="50" value={targetDpi} onChange={e => setTargetDpi(Math.max(50, Number(e.target.value) || 50))}/></label></div><div className="converter-result"><span>Matching sensitivity</span><strong>{(converterDpi * converterSensitivity / targetDpi).toFixed(3)}</strong></div></article></section>}
        ...
        {tab === "surface" && <section className="panel page-panel glass-surface"><div className="section-heading compact"><div><h2>Surface check</h2><p>A short pre-game check for movement and scroll behaviour.</p></div></div><article className="tool-card glass-control"><div className="surface-checks">{["Smooth tracking","Scroll movement","Lift-off check"].map(check => <button className={surfaceChecks.includes(check) ? "selected" : ""} onClick={() => recordSurfaceCheck(check)} key={check} type="button">{surfaceChecks.includes(check) ? "✓ " : ""}{check}</button>)}</div><small>{surfaceChecks.length ? `${surfaceChecks.length}/3 checks recorded` : "No checks recorded yet."}</small></article></section>}
