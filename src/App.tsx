  ...
  return <div className={`app-shell glass-${glassMode}`} style={glassStyle}>
    ...
    <div className="ui-scale-layer" style={{ "--ui-scale": uiScale / 100 } as React.CSSProperties}>
      <aside className="sidebar glass-surface">
        ...
        <section className="current-area" aria-label="Current device setup"><div className="sidebar-section-label">Current</div><section className="current-setup glass-control"><span>Current setup</span><strong>{activeProfile?.name || "Default"}</strong><div><b>{dpi} DPI</b><b>{polling}</b></div><small>{connectionLabel}</small><button onClick={() => changeTab("dashboard")} type="button">Open dashboard →</button><div className="sidebar-device-card glass-surface"><span className={`device-dot ${connected ? "online" : ""}`}/><div className="device-card-copy"><strong>{mouse?.name || "No mouse detected"}</strong><span>{connectionLabel}</span></div><button className="icon-button" onClick={() => void refresh()} type="button"><RefreshCw size={15} className={loading ? "spin" : ""}/></button></div></section></section><button className={`nav-item settings-nav ${tab === "settings" ? "active" : ""}`} onClick={() => changeTab("settings")} type="button"><Settings size={18}/><span>Settings</span></button>
