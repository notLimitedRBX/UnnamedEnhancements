...
type Tab = "overview" | "buttons" | "performance" | "profiles" | "trainer" | "tools" | "help" | "assistant" | "settings";
...
type TrainerMode = "Flick" | "Tracking" | "Reaction" | "osu-style";
...
const tabs: { id: Tab; label: string; icon: typeof Mouse }[] = [
  ...
  { id: "performance", label: "Performance", icon: Gauge }, { id: "profiles", label: "Profiles", icon: Gamepad2 }, { id: "trainer", label: "Aim trainer", icon: Zap }, { id: "tools", label: "Tools", icon: Zap }, { id: "help", label: "Help", icon: BookOpen }, { id: "assistant", label: "Local AI", icon: Bot }, { id: "settings", label: "Settings", icon: Settings },
];
...
export default function App() {
  ...
  const [trainerCombo, setTrainerCombo] = useState(0);
  ...
  useEffect(() => {
    ...
    const targetSpeed = trainerMode === "Tracking" ? 620 : trainerMode === "osu-style" ? 780 : 1000;
  ...
  const beginTrainer = () => {
    setTrainerScore(0); setTrainerCombo(0); setTrainerSeconds(30);
  ...
  return <div className={`app-shell glass-${glassMode}`} style={glassStyle}>
    ...
    <div className="ui-scale-layer" style={{ "--ui-scale": uiScale / 100 } as React.CSSProperties}>
      ...
      <main className="content">
        ...
        {tab === "trainer" && <section className="panel page-panel glass-surface trainer-panel"><div className="section-heading compact"><div><h2>Quick aim trial</h2><p>A standalone 30-second practice space for your current DPI.</p></div><span className="trainer-dpi">{dpi} DPI</span></div><div className="trainer-mode-row">{(["Flick","Tracking","Reaction","osu-style"] as TrainerMode[]).map(mode => <button className={trainerMode === mode ? "selected" : ""} onClick={() => { setTrainerMode(mode); setTrainerActive(false); }} key={mode} type="button"><b>{mode}</b><span>{mode === "Flick" ? "Fast target clicks" : mode === "Tracking" ? "Moving target practice" : mode === "Reaction" ? "Quick response" : "Rhythm circles & combo"}</span></button>)}</div><div className={`trainer-stage ${trainerMode === "osu-style" ? "osu-mode" : ""}`}><div className="trainer-score"><span>Hits <b>{trainerScore}</b></span><span>Time <b>{trainerSeconds}s</b></span>{trainerMode === "osu-style" && <span>Combo <b>{trainerCombo}</b></span>}</div>{trainerActive ? <button className="trainer-target" onClick={hitTrainerTarget} style={{ left: `${trainerTarget.x}%`, top: `${trainerTarget.y}%` }} type="button" aria-label="Aim target"><i>{trainerMode === "osu-style" ? trainerScore + 1 : ""}</i></button> : <div className="trainer-start"><strong>{trainerSeconds === 0 ? `${trainerScore} hits — nice run.` : "Ready when you are."}</strong><span>{trainerMode === "osu-style" ? "Click rhythm circles as they appear." : "Click the targets as quickly as you can."}</span><button className="primary-button" onClick={beginTrainer} type="button">{trainerSeconds === 0 ? "Try again" : "Start 30-second trial"}</button></div>}</div><p className="trainer-note">Practice happens only inside Unnamed. It does not read, modify, or automate any game.</p></section>}
