...
function normaliseHex(value: string, fallback: string) {
  ...
  if (/^[0-9a-fA-F]{3}$/.test(clean)) return `#${clean.split("").map((char) => char + char).join("").toUpperCase()}`;
  return fallback;
}

function HexColorControl({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  const [text, setText] = useState(value.replace(/^#/, "").toUpperCase());
  useEffect(() => setText(value.replace(/^#/, "").toUpperCase()), [value]);
  const commit = () => { const next = normaliseHex(text, value); setText(next.slice(1)); onChange(next); };
  return <label className="hex-color-control"><span>{label}</span><div className="hex-color-input"><input type="color" value={value} onChange={(event) => onChange(normaliseHex(event.target.value, value))} aria-label={`${label} colour picker`} /><span>#</span><input className="hex-input" value={text} maxLength={6} spellCheck={false} onChange={(event) => setText(event.target.value.replace(/[^0-9a-fA-F]/g, "").slice(0, 6))} onBlur={commit} onKeyDown={(event) => { if (event.key === "Enter") { event.currentTarget.blur(); } }} aria-label={`${label} hexadecimal value`} /></div></label>;
}
...
export default function App() {
  ...
  useEffect(() => { localStorage.setItem("unnamed-ui-scale", String(uiScale)); }, [uiScale]);
  ...
  return <div className="app-shell">
    ...
    <main className="content">
      ...
      {tab === "settings" && <section className="panel page-panel background-settings"><div className="section-heading compact"><div><h2>Appearance</h2><p>Customize the background and UI scale without changing the minimalist controls.</p></div></div>
        ...
        {backgroundMode === "solid" && <div className="background-control"><HexColorControl label="Background colour" value={backgroundColor} onChange={setBackgroundColor} /></div>}
        {backgroundMode === "gradient" && <div className="background-control gradient-controls"><HexColorControl label="Colour 1" value={gradientFrom} onChange={setGradientFrom} /><HexColorControl label="Colour 2" value={gradientTo} onChange={setGradientTo} /></div>}
        ...
        <div className="section-heading compact"><div><h2>Interface scale</h2><p>Make the app UI smaller to reveal more of your background, or larger for easier reading.</p></div><strong className="scale-value">{uiScale}%</strong></div>
        <div className="scale-control"><span>75%</span><input type="range" min="75" max="125" step="5" value={uiScale} onChange={(event) => setUiScale(Number(event.target.value))} aria-label="Interface scale" /><span>125%</span></div>
        <div className="scale-presets">{[75, 85, 100, 115, 125].map((value) => <button key={value} type="button" className={uiScale === value ? "selected" : ""} onClick={() => setUiScale(value)}>{value}%</button>)}</div>
